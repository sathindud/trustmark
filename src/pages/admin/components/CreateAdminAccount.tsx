...
const CreateAdminAccount: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await axios.post('/api/admin/account/create', {
        fullName,
        email,
        password,
      });
      ...
      setMessage(
        error.response?.data || 'Failed to create admin account'
      );
    }
  };
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-secondary-2">
      <div className="w-full max-w-md p-6 bg-white rounded-lg shadow">
        <h2 className="text-xl font-bold text-center mb-4">Create Admin Account</h2>
